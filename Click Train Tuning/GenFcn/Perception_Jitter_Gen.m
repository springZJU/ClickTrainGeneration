...
for jIndex = 1 : length(Jitter)
    ...
    if contains(soundType, ["Reg", "Irreg"]) % for regualr
    for bIndex = 1 :length(ICIBase)
        ...
        sounds((jIndex-1)*length(ICIBase) + bIndex).Wave =JitterClickGen(ICIBase(bIndex), S1Dur, Amp,  "repTail", repTail*repRatioTail, "fs", fs, "Jitter", jitterTemp, "JitterMethod", JitterMethod, "lastClick", lastClick);
    end
    end
end

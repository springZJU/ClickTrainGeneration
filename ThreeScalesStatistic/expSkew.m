...
while true
    % 生成正态分布数据
    originalData = normrnd(targetMean, targetStd, [N, 1]);
    ...
    % 验证数据是否满足特定条件
    if abs(currentMean - targetMean) / targetMean < 0.1 && abs(currentStd - targetStd) / targetStd < 0.08
        outputData = adjustedData;
        break; % 条件满足，退出循环
    end
    % 如果不满足条件，则循环继续，进行新一轮的生成和调整
end
outputData = improveSmoothness(outputData);
figure
histogram(outputData);
% 最终验证输出数据的统计特性
